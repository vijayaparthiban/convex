...
open(INFILE, "<$infile") or die("Couldn't read input file!\nUsage: perl GetEnsemblGenesTranscripts.pl /path/to/input_file.txt"); # INFILE as in CoNVex CNV calls output
...
while(my $line = <INFILE>) {
	...
	{    # Get all transcript names AND put them in a hash
		my $stable_transcript_id = $transcript->stable_id();
		if ( !$stable_transcript_id == '' ) {
			$trans_hash{$stable_transcript_id} = $nv;
		}
	}
	$all_genes .= "\t";

	for my $trans_key ( sort keys %trans_hash )
	{    # Append all transcript names to a single string, separated by ';'
		$all_genes .= $trans_key . ';';
	}

	$size = scalar( keys %trans_hash ); # If hash is empty, add 'NA' to the string -- this mean no known transcripts associated with this region
	if ( $size == 0 ) {
		$all_genes .= "NA";
	}

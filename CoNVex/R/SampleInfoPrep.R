...
`SampleInfoPrep` <- function(sample_ids,gender,bamfiles,RDfiles,L2Rfiles,GAMfiles,CNVfiles,sample_info_file,output_folder,overwrite=FALSE) {
	...
	if(length(which(checkbams==FALSE)) > 0) {
		write("BAM files are missing:", stderr());
		write(bamfiles[!checkbams], stderr());

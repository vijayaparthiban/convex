...
`SampleLogRatioV3` <- function(RDfiles, sample_ids, sample_gender, regions_file, chrX=0, cor_matrix_file="", min_samples=25, RPKM=0, sample_means_file="") {
	...
	if(cor_matrix_file=="") {
		cor_matrix_file = "CorMatrix.txt"
	}
	cor_matrix = cor(allXS[,8:ncol(allXS)]);
	write.table(cor_matrix,file=cor_matrix_file,row.names=FALSE, col.names=TRUE, sep="\t", quote=FALSE)
	allXS2 = allXS;
	...
	if(chrX == 0) {
		...
		for(i in 1:length(sample_ids)) { # for each sample's Chr X - males and females separately
			...
			cor_sample = sort(cor_matrix[,i], decreasing=TRUE); 
			#highly_correlated = names(cor_sample)[2:(min_samples+1)];
			if( any(Males == sid) ) { # If the sample is male
	 			cor_sample_males = cor_sample[(names(cor_sample) %in% Males)];
				highly_correlated = names(cor_sample_males)[2:(min_samples+1)];				
				...
				cor_sample_females = cor_sample[names(cor_sample) %in% Females_Unknown];
				highly_correlated = names(cor_sample_females)[2:(min_samples+1)];
